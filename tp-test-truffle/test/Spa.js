...
contract('Spa', accounts => {
    ...
    describe.skip("test du set et delete", function () {
        ...
        it("should set a new age for doggo", async () => {
            ...
            expect(storedData.age).to.be.bignumber.equal(new BN(4));
        });
    ...
    describe("test du adopt", function () {
        ...
        it("should adopt normaly, POV DOG", async () => {
            ...
            let storedTest =[ 'chien', '50', '7', true];
            expect(storedData.race).to.equal("chien");
            expect(storedData.isAdopted).to.be.true;
            expect(storedData).to.be.equal(storedTest);
        });
    ...
    describe.only("tests des event, du require, de revert", function () {
        ...
        xit("should adopt one, get event animal adopted", async () => {
        ...
        it("should not adopt max, too high criteria", async () => {
            const findReturn = await SpaInstance.AdoptIfMax("chat",10,1, {from:owner});
